...
assert_contains "command requires newcomer briefing" "$COMMAND" "newcomer-friendly overview"
assert_contains "command shows briefing with auto approval" "$COMMAND" "Even with the flag"
...
assert_contains "mastermind persists system overview" "$MASTER" "system-overview.md"
assert_contains "mastermind requires approval by default" "$MASTER" "Default plan policy is explicit user approval"
assert_contains "mastermind assumes newcomer audience" "$MASTER" "assume the user is new to the codebase"
assert_contains "mastermind explains runtime flow" "$MASTER" "end-to-end runtime, data, or control flow"
assert_contains "mastermind cites repository evidence" "$MASTER" "citing concrete repository paths"
assert_contains "mastermind orders overview before plan" "$MASTER" "Affected system overview"
assert_contains "mastermind asks only after briefing" "$MASTER" "only after both"
assert_contains "mastermind gates implementation" "$MASTER" "No implementation worker may start"
...
assert_contains "docs explain pre-implementation briefing" "$DOC" "Pre-implementation Briefing"
assert_contains "docs preserve briefing with auto approval" "$DOC" "does not skip that briefing"
