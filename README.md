...
custom agents in `.codex/agents/`. The active Subanana workflows are `$ticket-loop`, `$mr-loop`,
`$test-loop`, and `$double-check`; `$mission` and `$foreman` provide general parallel and
serialized plan execution.
